...
- README now gives the real Controller Manager path for the virtual D2,
  **Add... > Pre-mapped > Traktor Kontrol > D2**, replacing the Traktor Pro 3
  era **Add... > Traktor > Kontrol D2**. A new "No D2 in the device list"
  section covers installs where that menu path adds nothing and logs nothing:
  stock `CSI\D2` reproduces it, so the cause is in Traktor, and the way out is
  a `.tsi` import or moving `ApiModule {}` into a mapping the install loads.
  The failure is install-specific; the same path works on other machines on
