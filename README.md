...
3. Start Traktor Pro. No hardware is needed. If you don't own a Kontrol D2: **Preferences > Controller Manager > Add... > Pre-mapped > Traktor Kontrol > D2**. The "virtual" D2 mapping is what runs the mod. If no device appears, see [No D2 in the device list](#no-d2-in-the-device-list).
...
On some installs, **Add... > Pre-mapped > Traktor Kontrol > D2** does nothing at all: no
device joins the **Device** dropdown and `Traktor.log` records no error for the attempt.
Restoring the stock `CSI\D2` folder with the uninstall script reproduces it, so the cause sits
in Traktor rather than in the mod. It is install-specific, not a property of a Traktor
version: the same menu path on the same major version adds the device normally on other
machines. Seen on 4.5.1.21. The cause is not yet identified.
...
- **Import a `.tsi`.** On a machine that already has the D2 entry, select it in Controller
  ...
  Import builds the device from the file rather than from the menu path that fails.
